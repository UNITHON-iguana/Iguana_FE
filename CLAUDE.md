...
**검수는 사진 단위로 확정된다.** 확인할 칸이 없는 사진은 분석이 끝나는 즉시 스스로
검수 완료가 되어 집계로 넘어가고, 사람 손이 필요한 사진만 사진대지에 남는다.
무엇이 확정을 막는지는 `confirmBlocker`(`src/lib/workItems.ts`) 한 곳이 정한다 —
확인 필요 필터·자동 확정·검수 완료 체크박스가 모두 이 함수를 쓴다.
...
집계(`summary`)와 내보내기(`export`)는 **검수 완료된 사진만** 받아 쓴다.
확정되지 않은 사진은 합계에 섞이지 않고, 대신 몇 장이 빠져 있는지를 화면 위에 밝힌다.
집계는 공종마다 세는 단위가 다르고(배관은 개소, 덕트는 둘레 연장) 벽체 양면 시공 계수가
붙는데, **그 규칙을 프론트가 추측하지 않는다** — 품목 마스터가 백엔드에서 들고 온다.
자세한 것은 `docs/집계-규칙.md`.
